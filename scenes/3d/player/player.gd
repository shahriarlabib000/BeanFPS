...
var def_sensitivity := 0.02 * Autoload.camSensitivity/100
var sensitivity:=0.01
var bullet=preload("res://scenes/3d/bullet/bullet.tscn")
var deathScreen=preload("res://scenes/2d/death/death.tscn")
var zoom:=false
var flash:=false
var alive:=true
var openMenu:=false
...
@onready var onScreenMenu=$Head/Camera3D/onScreenMenu
@onready var viewPortSizeX=get_viewport().size.x
@onready var ui=$Head/Camera3D/ui
...
func _process(delta):
	...
	if alive:
		...
		if Input.is_action_just_pressed("onScreenMenu"):
			openMenu=!openMenu
			ui.visible=!openMenu
			onScreenMenu.visible=openMenu
			def_sensitivity = 0.02 * Autoload.camSensitivity/100
		...
		if(zoom):
			cam.fov=1
			sensitivity=def_sensitivity/50
		else:
			cam.fov=75
